...
*/
class Solution {
Node cloneGraphHelper(Node node,Set<Integer> set) {
        // code here
        if(node==null) return null;
        set.add(node.val);
        ArrayList<Node> ans=new ArrayList<>();
        
        for(var a : node.neighbors){
        if(!set.contains(a.val)){
            ans.add(cloneGraphHelper(a,set));
        }    
        }
        return new Node(node.val,ans);
    }
    Node cloneGraph(Node node) {
        Set<Integer> set= new HashSet<Integer>();
        return cloneGraphHelper(node,set);        // code here
